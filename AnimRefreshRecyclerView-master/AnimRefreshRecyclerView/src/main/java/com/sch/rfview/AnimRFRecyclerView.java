...
public class AnimRFRecyclerView extends RecyclerView implements Runnable {
    ...
    private boolean mTestModule = true;
    ...
    public void refreshComplate() {
        isLoadingData = false;
        if (mTestModule){
            mDialogContentView.setVisibility(GONE);
        }else {
    ...
    private void refresh() {
        ...
        if (mTestModule){
            if (null == mLoadingView) {
                ...
                ((ViewGroup) mHeaderViews.get(0)).addView(mDialogContentView, params);
            }else {
                mDialogContentView.setVisibility(VISIBLE);
            }
        }else {
            if (rfAnimView == null) {
                // 设置刷新动画
